...
func (h *Handler) SetupRoutes(cfg config.APIConfig) {
	...
	h.Server.Get("/v1/announcements", h.GetAnnouncements)
	...
	v1.Post("/user/delete/request", auth.NewStepUp(h.Service), limit.New(5, 10*time.Minute), h.DeleteUserRequest)
	...
	v1.Post("/user/stepup/passkey/finish", limiter.New(), h.StepUpPasskeyFinish)
	...
	v1.Post("/aliases/import", h.ImportAliases) // disabled `limit.New(5, 24*time.Hour)` for QA
